...
for (( i = 1; i <= $num_models ;  i++ )) {
    ...
    for (( rep_num = 0; rep_num < $nrep;  rep_num++ )) {
        echo "mpirun -np ${num_processes} $(pwd)/../../pb_mpi -d $(pwd)/../CV_data/${base_name}${rep_num}_learn.ali -s -x ${save_freq} ${chain_length} $(eval echo "$model_string") $(pwd)/../CV_data/${save_string}${base_name}${rep_num}_learn.ali" > submit_${save_string}_${rep_num}.sh
        qsub -q $(echo "$queue_var" | sed 's/^,//g') -wd $(pwd)/../CV_data -S /bin/bash -pe mpi ${num_processes} submit_${save_string}_${rep_num}.sh
    }
}
cd -
