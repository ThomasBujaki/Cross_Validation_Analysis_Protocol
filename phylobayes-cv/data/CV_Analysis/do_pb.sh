...
tree_file=$(grep "tree_file" analysis_parameters | awk '{print $2}')
...
for (( i = 1; i <= $num_models ;  i++ )) {
    ...
    for (( rep_num = 0; rep_num < $nrep;  rep_num++ )) {
        echo "$(pwd)/../../pb -d $(pwd)/../CV_data/${base_name}${rep_num}_learn.ali -T $(pwd)/../CV_data/${tree_file} -x ${save_freq} ${chain_length} $(eval echo "$model_string") $(pwd)/../CV_data/${save_string}${base_name}${rep_num}_learn.ali" > submit_${save_string}_${rep_num}.sh
